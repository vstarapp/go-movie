...
import (
	_ "go-movie/app/core/destroy" // 监听程序退出信号，用于资源的释放
	"go-movie/app/global/my_errors"
	"go-movie/app/global/variable"
	"go-movie/app/http/validator/common/register_validator"
	"go-movie/app/http/validator/common/validator_trans"
	"go-movie/app/service/sys_log_hook"
	"go-movie/app/utils/casbin_v2"
	"go-movie/app/utils/gorm_v2"
	"go-movie/app/utils/snow_flake"
	"go-movie/app/utils/websocket/core"
	"go-movie/app/utils/yml_config"
	"go-movie/app/utils/zap_factory"
	"log"
	"os"
)
...
func init() {
	...
	// 初始化验证器语言
	if err := validator_trans.LoadValidatorLocal(); err != nil {
		log.Println(err)
	}
